...
function shichang(){
    ...
    setTimeout(function(){
        ...
        var shichang={};
            ...
            $.ajax({
                type:"post",
                url:"https://data.bodevip.net.cn/api/collection/shichang",
                data:{
                    data:str
                },
                async:true,
...
function chitu(){
    var tit
    $(".left-menu-sub-title").each(function(){
        tit=$(this).text().replace(/\ +/g,"").replace(/[\r\n]/g,"")
        if(tit=="每日报表"){
            console.log($(this))
...
function ct(){
    var store;
    $("#text-username").each(function(){
        store=$(this).text().split(",")[1].split(":")[0].replace(/\ +/g,"").replace(/[\r\n]/g,"");
    ...
    $.ajax({
            type:"post",
            url:"https://data.bodevip.net.cn/api/collection/chitu",
            data:{
                data:all
            },
            async:true,
...
function spdate(){
    ...
    $(".screen-header .dtpicker-main .num").each(function(){
        ...
        $.ajax({
            type:"post",
            url:"https://data.bodevip.net.cn/api/collection/shangping",
            data:{
                data:data
            },
            async:true,
...
function liuliang(){
    ...
    setTimeout(function(){
        $("#content-container .oui-date-picker-current-date").each(function(){
            ...
            $.ajax({
                type:"post",
                url:"https://data.bodevip.net.cn/api/collection/flow",
                data:{
                    data:data
                },
                async:true,
    ...
    if(time=="100%"){
        ...
        $("#J_logListBox #J_logList tr:first a").eq(0).each(function(){
            var href=$(this).attr('href')
                ...
                $.ajax({
                type:"post",
                url:"https://data.bodevip.net.cn/api/collection/stock",
                data:{
                    data:data,
                },
                async:true,
...
function nextexpor(){
    ...
    setTimeout(function(){
        $(function(){
            $("#J_shopCatsBox .s-check .blue").each(function(){
                var name=$(this).text()
                 if(name.includes("上新")){
                    $(this).prev().click()
                 }
            ...
            setTimeout(function(){
                $("#J_logListBox #J_logList tr:first a").eq(0).each(function(){
                    var hrefs=$(this).attr('href')
                        ...
                        $.ajax({
                        type:"post",
                        url:"https://data.bodevip.net.cn/api/collection/match",
                        data:{
                            data:data,
                        },
                        async:true,
...
function qushu(){
    ...
    setTimeout(function(){
        ...
        $.ajax({
            type:"post",
            url:"https://data.bodevip.net.cn/api/collection/score",
            data:{
                data:s
            },
            async:true,
        ...
        if(name=="店铺数据"){
            ...
            setTimeout(function(){
                ...
                if(date==yesterdayTime){
                    ...
                    $.ajax({
                        type:"post",
                        url:"https://data.bodevip.net.cn/api/collection/qushu",
                        data:{
                            data:all
                        },
                        async:true,
...
chrome.runtime.sendMessage({greeting: '你好，我是content-script呀，我主动发消息给后台！'}, function(response) {
    ...
    if(switches==true){
        ...
        else if(url.includes("tb.maijsoft.cn/index.php?r=edit/index&error=access_denied")){
            setTimeout(function(){
                end()
            },1000)
        }
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
            if(request.greeting == "cookie"){
                ...
                setTimeout(function(){
                    $.ajax({
                        type:"post",
                        url:"https://data.bodevip.net.cn/api/collection/cookie",
                        data:{
                            data:s
                        },
                        async:true,
